 ...
 */
const updateTaskOrder = async (reqId, serviceGroup, txReq) => {
  // assert that txReq is instance of CreateTaskTxContent
  assert(
    new UpdateTaskOrderTxContent().instanceOf(txReq),
  ...
  serviceGroup.ipcService.sender("task/updateTaskOrder", reqId, true, {
    tid: txReq.tid,
